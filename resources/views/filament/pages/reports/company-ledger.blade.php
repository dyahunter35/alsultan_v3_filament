            ...
            @if($companyId)
                ...
                <div id="full-report-container">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6 mt-4 no-print">
                        <!-- المطلوبات (مدين) -->
                        <div
                            class="bg-gradient-to-br from-white to-slate-50 border border-slate-200 rounded-2xl p-5 shadow-sm hover:shadow-md transition-all border-r-4 border-r-blue-600">
                            <div class="flex justify-between items-start">
                                <div>
                                    <p class="text-xs font-bold text-slate-500 mb-1 uppercase tracking-wider">إجمالي
                                        المطلوبات (شحن)</p>
                        ...
                        <!-- المدفوعات (دائن) -->
                        <div
                            class="bg-gradient-to-br from-white to-slate-50 border border-slate-200 rounded-2xl p-5 shadow-sm hover:shadow-md transition-all border-r-4 border-r-rose-600">
                            <div class="flex justify-between items-start">
                                <div>
                                    <p class="text-xs font-bold text-slate-500 mb-1 uppercase tracking-wider">إجمالي
                                        المدفوعات (سداد)</p>
                        ...
                        <div
                            class="bg-gradient-to-br from-white to-slate-50 border border-slate-200 rounded-2xl p-5 shadow-sm hover:shadow-md transition-all border-r-4 border-r-amber-500">
                            <div class="flex justify-between items-start">
                                <div>
                                    <p class="text-xs font-bold text-slate-500 mb-1 uppercase tracking-wider">صافي المطلوب
                                        (الرصيد)</p>
                                    <h3
                                        class="text-2xl font-black text-slate-900 tabular-nums {{ $summary['final_balance'] >= 0 ? 'text-amber-600' : 'text-rose-600' }}">
                                        {{ number_format($summary['final_balance'], 2) }}
                                        @if($withRates)
                                            <span
                                                class="block text-sm font-bold mt-1 {{ $summary['final_balance_eq'] >= 0 ? 'text-emerald-600' : 'text-rose-600' }}">
                        ...
                        <table class="w-full ledger-table border-collapse" dir="rtl">
                                ...
                                @foreach ($report_lines as $index => $line)
                                    <tr class="hover:bg-slate-50">
                                        ...
                                        <td class="text-right px-4 font-medium  text-center">
                                            @if ($line['type'] == 'truck')
                                                <a href="#row-{{ $line['id'] }}"
                                                    class="text-blue-600 hover:underline">{{ $line['ref'] }}</a>
                                            @else
                                                {{ $line['ref'] }}
                                            @endif
                                        </td>
                            ...
                            <tfoot class="row-group-total">
                                <tr>
                                    ...
                                    <td class="tabular-nums bg-slate-800">{{ number_format($summary['total_debit'], 2) }}
                                    </td>
                    ...
                    <div id="print-section-2" class="mb-10 bg-white p-4 rounded-xl border border-gray-200 shadow-sm">
                        ...
                        <table class="w-full ledger-table border-collapse" dir="rtl">
                            <thead>
                                <tr>
                                    <th rowspan="3" class="bg-slate-200 border-slate-900 w-32 text-slate-900">التاريخ /
                                        البيان</th>
                                ...
                                @foreach($combined_records as $record)
                                    @if($record['type'] === 'truck')
                                        @foreach($record['cargos'] as $index => $cargo)
                                            <tr>
                                                @if($index === 0)
                                                    <td id="row-{{ $record['id'] }}" rowspan="{{ count($record['cargos']) + 1 }}"
                                                        class="bg-white font-bold border-r-2 border-slate-900">
                                                        <div class="mb-1">{{ $record['date'] }}</div>
                                                        <div class="no-print">
                                                            <a href="{{ \App\Filament\Resources\Trucks\TruckResource::getUrl('edit', ['record' => $record['id']]) }}"
                                                                target="_blank"
                                                                class="text-blue-600 text-[10px] flex items-center justify-center gap-1">
                                        ...
                                        <tr class="row-group-total">
                                            <td colspan="5" class="text-right px-4 italic text-xs">إجمالي الشحنة
                                                {{ $record['ref'] }}</td>
