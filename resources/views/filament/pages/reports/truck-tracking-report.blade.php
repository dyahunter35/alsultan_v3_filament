    ...
    <div id="report-content" class="bg-white p-2 print:p-0">
        ...
        <div class="flex justify-between items-start mb-6 border-b-4 border-slate-900 pb-4">
            ...
            <div class="w-1/3 text-left">
                <p>-</p>
                <p class="text-[10px] font-bold text-slate-700 leading-none mt-1">وقت التقرير:
                    {{ now()->format('H:i') }}</p>
                <div class="mt-2 text-[9px] bg-slate-900 text-white px-2 py-0.5 inline-block uppercase font-bold">
            ...
            <table class="w-full text-[11px] text-center border-collapse border border-slate-950 print:text-[10px]">
                <thead>
                    ...
                    <tr class="bg-slate-950 text-white">
                        ...
                        <th class="p-2 border border-slate-700 bg-emerald-900" colspan="8">تفاصيل الحمولة والمسار
                            الجمركي</th>
                    </tr>
                ...
                <tbody class="tabular-nums font-medium text-slate-900">
                    @forelse ($trucks as $index => $truck)
                        <tr class="hover:bg-slate-50 transition-colors border-b border-slate-300">
                            ...
                            <td class="p-2 border-x border-slate-300 font-black text-slate-900 uppercase">
                                {{ $truck->code ?? $truck->id }}</td>
                            <td class="p-2 border-x border-slate-200 text-right font-bold">
                                {{ $truck->contractorInfo?->name ?? 'N/A' }}</td>
                            <td class="p-2 border-x border-slate-200 text-[9px] text-slate-900">
                                {{ $truck->driver_name ?? 'N/A' }}<br>{{ $truck->driver_phone ?? 'N/A' }}</td>
                            <td class="p-2 border-x border-slate-200 text-[9px] text-slate-900">
                                {{ $truck->truck_model ?? 'تريلة 13 متر' }}</td>
                            <td class="p-2 border-x border-slate-200 font-black tracking-widest">{{ $truck->car_number }}
                            </td>

                            <td class="p-2 border-x border-slate-200 font-bold">{{ $truck->category?->name }}</td>
                            <td class="p-2 border-x border-slate-200 text-[10px]">
                                {{ $truck->country?->getLabel() ?? 'N/A' }}</td>
                            <td class="p-2 border-x border-slate-200 text-[10px]">{{ $truck->companyId?->name ?? 'N/A' }}
                            </td>
                            <td class="p-2 border-x border-slate-200">{{ $truck->city }}</td>
                            <td class="p-2 border-x border-slate-200 text-blue-700 font-bold">
                                {{ $truck->pack_date?->format('Y-m-d') }}</td>
                            <td class="p-2 border-x border-slate-200 text-rose-700 font-bold">
                                {{ $truck->arrive_date?->format('Y-m-d') ?? '-' }}</td>
                            <td class="p-2 border-x border-slate-200 bg-slate-50/50 font-bold">{{ $truck->toBranch?->name }}
                            </td>
                            <td
                                class="p-2 border-x border-slate-200 bg-amber-50/50 font-black text-amber-900 underline decoration-slate-400">
                                {{ $truck->from?->name }}</td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="14" class="p-20 text-center bg-slate-50">
        ...
        <div class="hidden print:grid grid-cols-4 gap-4 mt-16 px-4">
            ...
            <div class="text-center border-t-2 border-slate-200 pt-2">
                <span class="text-[9px] font-bold text-slate-400 uppercase block mb-12">مدير العمليات / Ops
                    Manager</span>
                <div class="border-b border-dotted border-slate-300 w-3/4 mx-auto"></div>
            </div>
            <div class="text-center border-t-2 border-slate-900 pt-2 bg-slate-50">
                <span class="text-[9px] font-black text-slate-900 uppercase block mb-12">ختم الإدارة / Official
                    Stamp</span>
            </div>
        </div>
