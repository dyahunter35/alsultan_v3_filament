    ...
    <div class="space-y-6" dir="rtl" >
        ...
        <div class="grid grid-cols-1 xl:grid-cols-4 gap-6">
            ...
            <div class="xl:col-span-1 space-y-4 print:hidden">
                <div class="p-5 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl shadow-sm">
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="space-y-3">
                            <label class="text-[10px] font-bold text-gray-400 uppercase tracking-widest px-1">أسعار الصرف الحالية</label>
                            @foreach($reportData['summary'] as $item)
                                <div class="flex flex-1 items-center justify-center gap-2 p-1 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg focus-within:border-primary-500 transition-all">
                                    <span class="text-xs font-bold text-gray-500 px-2 w-full truncate">{{ $item['currency_name'] }}</span>
            ...
            <div class="xl:col-span-3">
                <div class="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl shadow-sm overflow-hidden relative">
                    <div wire:loading wire:target="loadData, updateRate" class="absolute inset-0 bg-white/40 dark:bg-gray-900/40 backdrop-blur-[1px] z-10 flex items-center justify-center">
                        ...
                        <table class="w-full text-sm text-right border-collapse">
                            <thead>
                                <tr class="text-gray-500 border-b dark:border-gray-800">
                                    ...
                                    <th class="p-4 font-bold text-xs uppercase text-center bg-primary-50/30 dark:bg-primary-900/10 text-primary-600">المعادل بـ {{ $reportData['target_currency'] }}</th>
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-gray-50 dark:divide-gray-800">
                                @foreach($reportData['summary'] as $item)
                                    <tr class="hover:bg-gray-50/50 dark:hover:bg-gray-800/20 transition-colors">
                                        <td class="p-4 font-bold text-gray-700 dark:text-gray-300">{{ $item['currency_name'] }}</td>
                                        <td class="p-4 text-center font-mono text-gray-500">{{ number_format($item['total_claims'], 2) }}</td>
                                        <td class="p-4 text-center font-mono text-gray-500">{{ number_format($item['total_payments'], 2) }}</td>
                                        <td class="p-4 text-center font-mono font-bold {{ $item['total_balance'] < 0 ? 'text-danger-600' : 'text-success-600' }}">
                                            {{ number_format($item['total_balance'], 2) }}
                                        </td>
                                        <td class="p-4 text-center font-mono font-bold text-primary-700 dark:text-primary-400 bg-primary-50/20 dark:bg-primary-900/5">
                            ...
                            <tfoot class="bg-gray-50 dark:bg-gray-800/50">
                                <tr>
                                    <td colspan="4" class="p-4 text-left font-bold text-gray-400 text-xs">إجمالي المحفظة المالية</td>
                                    <td class="p-4 text-center font-mono font-black text-lg text-primary-600">
                                        {{ number_format($grandTotalEquivalent, 2) }}
                                        <span class="text-[10px] font-bold text-gray-400 mr-1">{{ $reportData['target_currency'] }}</span>
                                    </td>
                                </tr>
                            </tfoot>
                        </table>
        ...
        <div class="space-y-6 pt-4">
            <h3 class="text-xs font-black text-gray-400 uppercase tracking-[0.2em] border-r-4 border-primary-500 pr-3">تفاصيل الأرصدة لكل عملة</h3>

            @foreach($reportData['groups'] as $currencyId => $group)
                <div class="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl shadow-sm overflow-hidden">
                    <div class="p-4 flex items-center justify-between bg-gray-50/50 dark:bg-gray-800/30 border-b dark:border-gray-800">
                        <div class="flex items-center gap-3">
                            <span class="px-2 py-1 rounded bg-gray-800 text-white text-[10px] font-black">{{ $group['currency_code'] }}</span>
                            <h2 class="text-sm font-bold text-gray-800 dark:text-white">حسابات {{ $group['currency_name'] }}</h2>
                        </div>
                        ...
                        <table class="w-full text-xs text-right border-collapse">
                            <thead>
                                <tr class="text-gray-400 border-b dark:border-gray-800">
                                    ...
                                    <th class="p-3 text-center border-x dark:border-gray-800">الرصيد ({{ $group['currency_code'] }})</th>
                                    <th class="p-3 text-center bg-gray-50 dark:bg-gray-800/50 font-bold">المعادل بـ ({{ $reportData['target_currency'] }})</th>
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-gray-50 dark:divide-gray-800/50">
                                @foreach($group['companies'] as $idx => $company)
                                    <tr class="hover:bg-gray-50/30 dark:hover:bg-gray-800/10 transition-colors">
                                        ...
                                        <td class="p-3 font-bold text-gray-700 dark:text-gray-300">{{ $company['name'] }}</td>
                                        <td class="p-3 text-center font-mono text-gray-500">{{ number_format($company['claims'], 2) }}</td>
                                        <td class="p-3 text-center font-mono text-gray-500">{{ number_format($company['payments'], 2) }}</td>
                                        <td class="p-3 text-center font-mono font-bold border-x dark:border-gray-800 {{ $company['balance'] < 0 ? 'text-danger-600' : 'text-success-600' }}">
                            ...
                            <tfoot class="border-t border-gray-200 dark:border-gray-700 bg-gray-50/30 dark:bg-gray-800/20">
                                <tr class="font-bold">
                                    <td colspan="2" class="p-3 text-left text-gray-400">المجاميع الفرعية</td>
                                    <td class="p-3 text-center font-mono">{{ number_format($group['totals']['claims'], 2) }}</td>
                                    <td class="p-3 text-center font-mono">{{ number_format($group['totals']['payments'], 2) }}</td>
                                    <td class="p-3 text-center font-mono text-sm border-x dark:border-gray-800 {{ $group['totals']['balance'] < 0 ? 'text-danger-600' : 'text-success-600' }}">
                                        {{ number_format($group['totals']['balance'], 2) }}
                                    </td>
                                    <td class="p-3 text-center font-mono text-sm text-primary-700 dark:text-primary-400 bg-primary-50/30 dark:bg-primary-900/10">
        ...
        <style>
            ...
            .fi-main { font-family: 'Almarai', sans-serif !important; }
            ...
            @media print {
                ...
                .border, .border-b, .border-x { border-color: #f1f1f1 !important; }
