                    ...
                    <tbody class="divide-y divide-slate-200 tabular-nums font-medium">
                        @foreach ($ledger as $row)
                            <tr class="transition-colors hover:bg-slate-50/80">
                                ...
                                <td class="p-2 font-bold border border-slate-200 text-slate-700">{{ $row['transaction_name'] }}
                                </td>
                        ...
                        <tr class="font-black border-t-2 bg-slate-800 text-white border-slate-900">
                            ...
                            <td class="p-2 border bg-blue-700/50">{{ number_format($ledger->sum('customer_sales'), 2) }}
                            </td>
                            <td class="p-2 border bg-blue-700/50">{{ number_format($ledger->sum('customer_payment'), 2) }}
                            </td>
                            <td class="p-2 text-lg border bg-blue-900">
                                {{ number_format($ledger->last()['customer_balance'] ?? 0, 2) }}</td>

                            <td class="p-2 border bg-green-700/50">{{ number_format($ledger->sum('treasury_debit'), 2) }}
                            </td>
                            <td class="p-2 border bg-red-700/50">{{ number_format($ledger->sum('treasury_credit'), 2) }}
                            </td>
                            <td class="p-2 text-lg border bg-green-900">
                                {{ number_format($ledger->last()['treasury_balance'] ?? 0, 2) }}</td>
                        </tr>
                    </tbody>
