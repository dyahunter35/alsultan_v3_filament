<x-filament-panels::page>
    <x-filament::section class="mb-4 shadow-sm no-print border-slate-200">
        <div class="flex flex-col gap-4 md:flex-row md:items-end">
            ...
            <x-filament::button wire:click="loadData" color="gray"
                icon="heroicon-m-arrow-path">تحديث</x-filament::button>
        </div>
    </x-filament::section>
    ...
    @if ($supplyings->count())
        <div id="report-content" class="space-y-6 print:m-0">
            ...
            @if($customer)
                <div
                    class="grid grid-cols-3 overflow-hidden bg-white border shadow-sm md:grid-cols-3 rounded-xl print:shadow-none print:border-slate-300">
                    ...
                    <tbody class="divide-y divide-slate-200 tabular-nums">
                        @foreach ($supplyings as $row)
                            <tr class="hover:bg-slate-50">
                                <td class="p-2 border border-slate-200 text-slate-500">{{ $row->created_at?->format('Y-m-d') }}
                                </td>
                                @if(!$customerId)
                                ...
                                <td class="p-2 border border-slate-200">
                                    <span
                                        class="px-2 py-1 text-xs bg-slate-100 rounded-full">{{ $row->payment_method->getLabel() }}</span>
                                </td>
                                <td class="p-2 border border-slate-200 text-slate-400 italic">
                                    {{ $row->payment_reference ?? '—' }}</td>
                        ...
                        <tr class="font-black bg-slate-100 border-t-2 border-slate-800">
                            ...
                            <td colspan="{{ ($customerId ? '5' : '6') }}" class="p-3 text-left px-6">إجمالي التوريدات في هذه
                                الفترة</td>
